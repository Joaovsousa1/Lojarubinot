...
export function AppProvider({ children }) {
  ...
  const [accountsTab, setAccountsTabRaw] = useState(
    () => sessionStorage.getItem('rubinot_accounts_tab') ?? 'contas'
  )
  const setAccountsTab = useCallback((tab) => {
    sessionStorage.setItem('rubinot_accounts_tab', tab)
  ...
  function dbToNote(row) {
    return {
      ...
      dueAt: row.due_at, accountId: row.account_id ?? '', loyaltyId: row.loyalty_id ?? '',
      email: row.email ?? '',
  ...
  function noteToDb(note) {
    return {
      ...
      due_at: note.dueAt || null, account_id: note.accountId || null, loyalty_id: note.loyaltyId || null,
